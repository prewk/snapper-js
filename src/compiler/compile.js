...
function toUpdateTask(
    task: CreateTask,
    schema: Entity,
    problematicColumns: Array<string>
): UpdateTask {
    const updateColumns = [];
    const updateValues = [];
...
function compile(
    ...
    const createTasks: Array<CreateTask> = [];
    const updateTasks: Array<UpdateTask> = [];

    // Pre-compile optimization - sort the entities
    const unsortedCreateTasks = entities.map((entityRow: EntityRow) =>
        makeCreateTask(idMaker, getEntityByName(schema, entityRow.name), entityRow)[0]
    );

    entities = sortByMostRequired(idMaker, entities, unsortedCreateTasks);

    // Compile
    entities.forEach((entityRow: EntityRow) => {
