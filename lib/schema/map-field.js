...
function validate(schema, fields, entities) {
    ...
    if (Array.isArray(value) || isPlainObject(value)) {
        ...
        const keyLookup = entities.reduce((keyLookup, entity) => {
            const assignee = {};
        ...
        const dependencies = flattenDeep(schema.relations.map(relation => getMapEntryDependencies(relation, value, dotMap)));
...
function transform(schema, fields, transformer) {
    ...
    if (fields.hasOwnProperty(schema.name)) {
        ...
        // Gather all dependencies in this map
        const deps = flattenDeep(schema.relations.map(relation => getMapEntryDependencies(relation, map, dotMap)));
